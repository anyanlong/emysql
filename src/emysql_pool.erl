...
%%%
%%% @end
%%% Created : 10 Apr 2014 by Jack Tang <jack@taodinet.com>
%%%-------------------------------------------------------------------
-module(emysql_pool).
...
%%--------------------------------------------------------------------
pool(Id) ->
    gen_server:call(?SERVER, {pool, Id}).
...
%% @end
%%--------------------------------------------------------------------
handle_call({pool, Id}, _From, #state{pools = Pools}=State) ->
    Reply = case lists:member(Id, Pools) of
                true  -> {ok, ok};
...
%% @end
%%--------------------------------------------------------------------
terminate(_Reason, #state{pools = Pools} = State) ->
    lists:foreach(
      fun(Pool) ->
